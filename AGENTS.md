...
- **NachOS refuses only what it is sure of.** A refusal that is wrong costs the bot an order the game would have
  taken and cannot be undone within the turn, where letting one through costs nothing the game does not already
  cost it. Where NachOS judges an order itself — `api.order.budget` — it leans on what this step's observation
  states, and every such refusal has a way past it.
