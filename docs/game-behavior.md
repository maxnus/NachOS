...
What StarCraft II does, as NachOS has seen it through the API. Observed with StarCraft II 5.0.15, base build 95841,
unless an entry says otherwise. A fact that shapes one piece of code is also in that code's docstring; this page is
where to look one up, and where a new finding goes. [cheats.md](cheats.md) covers cheats, and
[curating-ids.md](curating-ids.md) how the game's ids were sorted into the curated ones.

Each entry ends with how it was seen:

- **#n**: reproduced in a running game by the probes or tools of pull request #n, whose description says how;
- **in game**: seen in a running game, with no record of which pull request;
- **tested**: an in-game test (`uv run pytest -m integration`) still checks it;
- **tool** *name*: `tools/`*name*`.py` reproduces it in a game and records it;
...
- The game serves one websocket connection at a time and drops a second during its handshake. Once the first
  closes, a new connection finds the game the first created, and after that game a ping reports `launched` (#14;
  tested).
- Responses carry nothing that matches them to a request: the game answers the last request sent, so only one can
  be open at a time (#6).
- Every response carries a status. An unset proto2 enum field reads as its first declared value, not zero: an unset
  ...
  truthy, so each is read only behind `HasField` (#6, #9).
...
- Leaving a running or ended game concedes it (the opponent is told it won) and returns the client to `launched`;
  leaving when no game was started is refused. An ended game accepts a new `create_game` without being left first
  (#14).
...
- In a game of two clients, a step passes only once both have requested it, and each join waits for the other (tool
  `_sandbox`, #37).
...
- The executable is `Versions/Base<build>/SC2_x64.exe` on Windows, started from `Support64`, `SC2_x64` on Linux, and
  `SC2.app/Contents/MacOS/SC2` on macOS. The launcher records a non-default install location in `ExecuteInfo.txt`,
  under `Documents/StarCraft II` on Windows and `Library/Application Support/Blizzard/StarCraft II` on macOS, as
...
- A client left running keeps hold of the graphics card (stated).
- Every map in use seats two. The game silently drops extra players, and `PlayerSetup` cannot set teams (#11).
  `create_game` ignores a participant's race and name: the join sets them. `PlayerSetup.race` counts only for a
  computer (#9).
...
- A game with no opponent is won the moment it starts (#22).
...
- Chat, commands and deaths from 45 unobserved realtime steps all arrived in the next observation, and never again:
  each is reported once, in the first observation after it, however many steps that spans (#28).
- On the ladder or in realtime, an order's effect can show up an observation later than in a stepped game, and
...
- Every debug cheat is a toggle for the whole game: sent twice, it is off, and in a game of two only one player
  sends it (#28, #37).
- The cheats take a few steps to take effect; the in-game tests wait 8 (tested).
...
- `god` stops this player's units taking damage and multiplies every weapon's damage in `ResponseData` by 10. Its
  effect on the tables is absent in a game's first ten steps, shows within 4 steps once 60 have run, and is gone
  within 16 of turning it off (#30).
- `food` lifts the supply cap (in game).
- A debug-made unit can appear a few steps late, and the game does not always make what is asked: not every pylon,
  and no structure on ground it cannot stand on; a photon cannon ordered onto this player's creep was never made
  (tool `sweep_alerts`, #37).
- A unit made for player 0 is reported as owner 16 (tool `_sandbox`).
- Debug commands can make a larva but not a plain creep tumor, and a queen's tumor only on creep (#28).
...
- A vital set to 0 by debug command goes to its maximum instead: shields stay full, and energy reads 200 (#41, tool
  `sweep_orders`).
...
- A structure or mineral field that leaves sight is replaced, in the same observation, by a remembered copy under a
  new tag at a bit-identical position; seen again, it is back under its first tag, and each return to the fog makes
  another tag. Mineral fields start the game as such copies (#25; tested).
- A copy can have a different type from the structure it copies: a depot that lowered as it left sight (#25).
...
- Map-owned units are owner 16 (stated). An enemy structure built out of sight is not listed (tested).
...
- A larva stays one unit as it becomes an `EGG`. What hatches comes under new tags, two zerglings as two units, and
  the egg is reported dead (#37; tested). The death comes in the first observation without the egg's order, so a
  larva's order ends with its egg dying, and an egg killed before it hatches looks the same (tested).
...
- A drone that becomes a structure leaves the observation with no death reported, and the structure appears under a
  new tag. The game reports the drone dead when the structure finishes or is killed, in the same step in 3 games of
  6 and a step later in the other 3. On a cancel the structure is reported dead and the drone returns in that
  observation, under its own tag and with its health (#25, #37; tested).
- An SCV first walks to the site carrying the build order; once construction starts, its order targets the
  structure's snapped center, at most 0.71 from the ordered point. Halted, it has no orders and progress stops;
  resumed with a smart order, its order targets the structure; killed, it is reported dead and progress freezes;
  once the structure is finished, it has no orders (#25).
- A probe warps its building in and walks away, and a drone becomes the building, so only an SCV has anything to
  halt (in game).
- A worker that built a gas structure goes inside it to harvest for under 120 steps (tool `sweep_tech_tree`).
- Without `fast_build` a supply depot is still going up 120 steps after the order, and done within 600 more
  (tested).
- An add-on is a unit of its own, first seen the step after the order at 2% built. It sits 2.5 right of its host and
  0.5 below; with no room the host lifts off to build it elsewhere (#37; tested). Ordered without a point, as a bot
  usually orders one, it is built where the host stands and charged at once, 50/50 for a reactor; but a host whose
...
- A structure cancelled while going up is reported dead, and so is a MULE when it expires (#37; tested).
- A neural parasite keeps the tag and hands the unit to the caster's player within 24 steps, alliance and upgrades
  included (a marine read 55 health), with the `INFESTOR_NEURAL_PARASITE` buff; the unit returns when the infestor
  dies (#25, #34; tested).
...
- Vision of an enemy lingers some 60 steps after the unit that saw it dies (#37; tested).
- An enemy that cloaks where it stands stays listed. An undetected enemy observer is listed `CLOAKED` and
  ...
  and `IN_VISION`, and a ghost detected that way shows `GHOST_CLOAK` (#37; tested).
- A cloaked unit of this player's (an observer, a dark templar, a cloaked ghost) reads `CLOAKED_ALLIED` and
  ...
  and shot one (the turret took an observer from 70 to 34), and nothing in its report changed (#37).
- A ghost cloaks and uncloaks one step after the order (#37; tested).
...
- The visibility grid reads 0 for never seen, 1 for seen before and 2 for in sight now; creep is one bit per tile.
  At the start this player's base has creep and vision, and the enemy's start is unexplored (tested).
...
- A colossus lands 2 hits per attack (stated). Range is measured horizontally (stated).
- Every unit below its maximum energy regenerates some every step (#37).
...
- A unit under attack that cannot fight back drifts away unless it holds position (tool `sweep_alerts`).

## Buffs

- A melee game puts 45 buffs on units, 5 of them for carrying resources: minerals under one buff for every race and
  another for rich minerals, gas under a buff per race's worker. Map zones put buffs on units too (tool
  `sweep_buffs`, #24).
- Workers gain and lose a buff on every mineral trip, picking up and delivering: all but 14 of the corpus's 16,882
  buff changes are those (corpus, #37).
...
- `BuffData` has only an id and a name, and the name is the id's own (#23).
...
- `ResponseData` asked after an upgrade folds the asking player's upgrades, and only theirs, into the rows: 131 of
  ...
  tested).
- The rows include damage, bonuses (new ones too: Infernal Pre-Igniter gives the hellbat a bonus against light),
  range, armor and speed, each added on. Every level adds the same amount to a type, but different types get
  different amounts: a marauder gets +1 and +1 against armored (tool `sweep_upgrades`, #30).
...
- The game reports no enemy upgrade beyond the attack, armor and shield levels on its visible units, and a level
  belongs to the player's whole upgrade line; an ultralisk's armor is ambiguous, and Metabolic Boost is never
  reported. Other upgrades can be inferred from what is seen: warp gates, burrow, stim, charge, lunge, cloak, a
  55-health marine, a concussive slow, an interference matrix, a storm (#31, #34).
...
- `RequestData` names no performer for any ability, gives the ghost, thor, battlecruiser and mothership no
...
- The `available` flag filters nothing: `MorphZerglingToBaneling` is marked unavailable though zerglings morph, and
  the viking's alias 1940, an empty row no unit ever has as its type, is marked available (#23).
- A morph's row prices it with what it came from (an orbital command 550 minerals, the command center's 400
  included), though the game charges only the difference; its build time covers only the morph (25 s). A consumed
  performer's cost is folded in: an extractor costs 75. Costs are whole numbers, and zerglings come in pairs (#23;
  corpus).
- What the game charges when an ability is ordered is the product's row less the row of what the product is made
  from, and that holds for every creation ability but six. A hatchery is charged 300 where that gives 275, its row
  reading 325 rather than the drone's 50 plus a hatchery's 300; a warp gate nothing, though its row holds a
  gateway's 150 and records nothing it is made from; an overlord transport 25/25, though its row reads the same 100
  as an overlord's; a pair of zerglings 50 minerals and a whole supply, where the row prices one; an auto turret
  nothing but the raven's energy, its row keeping a price the game no longer charges; and a purification nova no
  supply, though its row carries the disruptor's 3. A nuke, 100/100, and an interceptor, 15, have no row to derive
  from at all, since the curated ids name no unit type for either. A reactor and a tech lab no longer need a
  correction of their own: their rows read 50/50 and 50/25 (corpus).
- `unit_alias` names the type something morphed from; `is_building` means "needs placing"; an add-on's
  `footprint_radius` is 3.5, the reach past its host's far side; a bare tech lab or reactor is a requirement, and no
  unit is ever built as one (#23).
...
- With no supply, an order is refused as given: drones get `NotEnoughFood`. Five SCVs queued with 2 supply left
  train 2, and the observation with the second carries a `NotEnoughFood` action error, as it does when the depot
  they need is killed. A depot ordered onto minerals gets `CantFindPlacementLocation`, and one whose ground a sieged
  tank then took gets a `CantBuildLocationInvalid` action error (tool `sweep_alerts`, #37).
- A second SCV or drone ordered with no minerals left, queued or not, is answered `Success`, never made, raises no
  ...
  `sweep_orders`).
- Supply is taken when a unit starts, not when it is queued, and a full cap stalls the queue rather than refusing
  it. A barracks with two supply left given five marines takes all five: `food_used` rises by one as the first
  starts, and the four behind it wait at no progress, taking nothing. One given three with the cap full takes all
  three, answers each `Success`, and starts none: no progress for 380 steps, no action error, and nothing dropped.
  Two SCVs queued at once take one supply between them, and cancelling the one that had not started gives none
  back (tool `sweep_orders`).
...
- An item in the middle of a queue can be cancelled, but only the same way: the UI action
  `ActionProductionPanelRemoveFromQueue`, with the structure selected. A barracks holding a marine, a reaper, a
  marine, a reaper and a marine, selected by a rally, and asked for index 2: answered `Success`, the third item
  ...
  `CancelSlot_Queue5` and `CancelSlot_QueueCancelToSelection` `NotSupported`. So a selection is not what those ids
  were missing, and NachOS, which asks for the raw interface alone, can cancel only a structure's last item (tool
  `sweep_orders`).
...
- Every action in a `RequestAction` gets a verdict, in the order sent, even 500 in one request, and the game carries
  them out in that order: of two unqueued moves to one unit the second stands, an unqueued move after a queued one
  clears it, and a queued move after an unqueued one goes behind it. A stim and a move to one marine are both
  carried out in either order: it moves, stimmed (tool `sweep_orders`).
- Training and research queue behind what a structure is already making, queued or not: an unqueued train given to
  a command center, hatchery or nexus that is training, or an unqueued research to an engineering bay, forge or
  evolution chamber that is researching, waits its turn like a queued one. A structure holds 5; a sixth is answered
  `QueueIsFull`. A structure with a reactor holds 8 and makes two at once, barracks, factory and starport alike; one
  with a tech lab or no add-on holds 5 and makes one. Given 10 in one step, each kept its 8 or its 5, and exactly 2
  or 1 of them showed progress (tool `sweep_orders`).
- A morph or an add-on given to a structure that is making something is refused, `NotSupported`, queued or not: an
  ...
  morphs and an idle barracks builds its tech lab. A gateway turning into a warp gate by itself after the research
  waits for its zealot to finish (tool `sweep_orders`).
- A barracks stays busy one step longer than its add-on takes: in the observation where the tech lab or reactor
  first reads finished, the barracks still shows the order that built it, is offered no train, and refuses a marine
  `NotSupported`. A step later it takes one (tool `sweep_orders`).
- Each action is judged against the game state as the last step left it, not as earlier actions in the same step
  change it. When the game steps, it carries them out in order and drops what no longer fits, with no error and no
  entry among the reported actions. Ten marines sent to a barracks in ten requests within one step are all answered
  `Success`: five are queued, or eight with a reactor, and the rest are dropped. Of seven SCVs in one request to a
  command center, five are queued (tool `sweep_orders`).
- Through `RequestAction`, a structure's production can be cancelled only from its last item. `Cancel_Last` and a
  structure's own cancel remove the last item, and a busy structure is offered no other cancel: `Cancel_Queue5` for
  a barracks or a bay, `Cancel_QueueCancelToSelection` for a command center. `Cancel_Slot` and the generic `Cancel`
  are answered `Error`, and another structure's cancel and `CancelSlot_Queue5` `NotSupported`. The reported action
  names the structure's own cancel, whichever was sent. A cancel to an idle structure is answered `Error`; three
  cancels to a barracks training two marines are all answered `Success`, and two are carried out (tool
  `sweep_orders`).
- A cancel makes no room for what follows it in the same step. A cancel and then a marine to a barracks holding 5:
  the cancel is carried out and the marine refused `QueueIsFull`; a step later a marine is taken. To one holding 4,
  a cancel and then a reaper are both carried out, and the reaper goes last. Cancels followed by an orbital command
  to a command center training SCVs, or a cancel followed by a tech lab to a barracks training a marine, empty the
  queue and refuse the morph or add-on `NotSupported`; a step later it is taken (tool `sweep_orders`).
- A cancel's refund follows a fixed rule and does not depend on progress: all of a train or a research, three
  quarters of a morph, an add-on or a structure going up, rounded up. Seen at real prices, as the jump in the bank
  over the observation the cancel landed in: an SCV 50 of 50 and a research 100/100 of 100/100; an orbital command
  113 of 150, a planetary fortress 113/113 of 150/150, a tech lab 38/19 of 50/25, and a supply depot 75 of 100. An
  orbital cancelled 400 steps into its morph refunds the same 113 as one cancelled 40 steps in, and a depot
  cancelled half way up the same 75 as one barely started (tool `sweep_orders`).
- A structure part way through something is offered exactly one cancel, its own. A command center morphing is
  ...
  `Error` by a morph and by an add-on, so send the cancel the game offers, not the generic one (tool
  `sweep_orders`).
- Which cancel a structure is offered depends on what it is making; every producer of the three races was read. A
  command center morphing is offered `Cancel_MorphOrbital` or `Cancel_MorphPlanetaryFortress`, according to the
  morph it is running; a barracks, a factory and a starport building either add-on `Cancel_BarracksAddOn`,
  `Cancel_FactoryAddOn` and `Cancel_StarportAddOn`; a hatchery morphing `Cancel_MorphLair`, a lair `Cancel_MorphHive`
  and a spire `Cancel_MorphGreaterSpire`. Everything training or researching is offered the queue cancel of its
  ...
  on a barracks, a factory and a starport alike. A warp gate, which keeps no queue, is offered none. A cancel is
...
- A cancelled SCV refunds its 50 minerals by the next observation, whether half made or only queued, and the refund
  ...
  it. With none, 5 cancels to a command center and an orbital command to another, idle one: the orbital is refused
  `NotEnoughMinerals`, and a step later the refunded 250 pay for it (tool `sweep_orders`).
- Every non-producing ability a producing structure is offered leaves what it is making alone, at its progress.
  Each was given to a structure part way through a train or a research, and the structure carried on: the rally of a
  barracks, factory, starport, robotics facility, stargate, nexus, command center, orbital command, planetary
  fortress, hatchery, lair and hive, for workers and units alike; a command center's and a planetary's load; an
  orbital's MULE, scan and supply drop; a nexus's recall and energy recharge; a ghost academy's nuke; and a
  planetary fortress's own stop and attack, for which a barracks is answered `Error` (tool `sweep_orders`).
...
- A research that another of this player's structures is already doing is answered `Success`, and nothing is
  ...
  `sweep_orders`).
- A structure mid-research is offered no research at all, its own included, though it takes every other line
  ...
  missile 1 and ground armor 1, each refusing `NotSupported` the second level of a line it was already on (tool
  `sweep_orders`).
- A few abilities act at once and leave a unit's orders as they were, so a move carries on: stim, the banshee's and
  ...
  at a unit or a point, even one within reach. Queued, each goes behind the move, except a cyclone's lock-on, which
  is refused `NotSupported` (tool `sweep_orders`).
- The off half of a toggle keeps a unit's orders too, like the on half. A unit is offered the off half only once the
  on half has taken effect, and each was then given to a moving unit: the banshee's and ghost's cloak, the ghost's
  hold fire, the oracle's pulsar beam, the overlord's creep and the baneling's attack on structures. Every one is
...
- One command to several units sends each moving unit to its own point around the one ordered, so they keep their
  ...
  given one train (50 minerals went and one of the three carried the order), one research from two engineering bays
...
- A larva keeps no queue. Given a drone, an overlord and a drone in one request, all three answered `Success`, it
  becomes an egg carrying only the last of them, and the hatchery's own orders stay empty. Every larva of a hatchery
  given a drone becomes a drone egg, and a second order to a larva already spoken for changes nothing (tool
  `sweep_orders`).
...
- A builder killed before the game gives up on its build is reported dead and nothing more. An SCV whose site one of
  this player's marines held position on was given up on 108 steps after the order, with `CantBuildLocationInvalid`;
  one sent to another such site and killed on the way got no error at all, then or twelve steps later (tool
  `sweep_orders`).
- A structure killed while making something is reported dead and nothing more. A barracks training three marines,
  killed: the next observation still lists it with all three orders, the one after does not list it at all, and
  neither carries an action or an action error for it. An SCV killed on its way to build is the same. So what a dead
  unit was making is known only from the unit being gone, never from a report (tool `sweep_orders`).
- An enemy structure out of sight is attacked by the tag of the snapshot the observation lists for it; the tag it
  was seen under is refused `NotSupported` (tool `sweep_orders`).
- A point crosses the protocol as a 32-bit float, so a coordinate the game reports is exactly a float, widened; a
  point sent is rounded to a float on the way out, before the game rounds it down to its own lattice. The two
  roundings differ: x = 157.123288015625 goes out as 157.123291015625, a whole lattice step above where rounding
  down alone would land (tool `sweep_orders`; stated).
- The game keeps a point to 1/4096, rounded down: a move to x = 157.123456 is carried out and reported as
  157.123291, and one to 157.124456 as 157.124268. A builder's order shows its structure's site snapped to where the
  structure will stand, on a tile corner for an even footprint and a tile center for an odd one: a depot ordered at
  (86.3, 159.7) at (86, 160), a barracks ordered at (82.3, 158.7) at (82.5, 158.5). The reported action keeps the
  point as sent (tool `sweep_orders`).
- An unqueued order identical to a unit's first, in the ability it runs and the target to 1/4096, is ignored:
  answered `Success`, left out of the reported actions, and free. Four marines attacking pylons, each standing at
  each of four sites in turn for 896 steps, dealt 1382 to 1402 in all whether their attack was sent once or re-sent
  every step, every 4th or every 16th, each round within two shots of the others; a re-sent move covered the same
  ground, and a gather re-sent while the worker gathered mined as much. It still drops the orders queued behind the
  first. An order that differs is carried out: a gather sent to a worker returning its cargo sends it back to the
  field with the cargo, so one re-sent every 16 steps mined 5 minerals in 1344 steps against 65 (tool
  `sweep_orders`).
- A builder whose site one of this player's units holds position on gets a `CantBuildLocationInvalid` action error
...
- An alert names no unit and no position. It comes once per event, in the observation where that event is first
  seen, and several can share an observation (tool `sweep_alerts`, #37; tested).
- `TRAIN_WORKER_COMPLETE` once per worker; `TRAIN_UNIT_COMPLETE` once per non-worker unit, twice for a pair of
  zerglings, and never for a mothership, a warp-in or a morph; `MOTHERSHIP_COMPLETE` instead for a mothership;
  `WARP_IN_COMPLETE` once per warp-in (tool `sweep_alerts`, #37; tested).
- `BUILDING_COMPLETE` once per structure, including one a drone became and a nydus worm, but not an add-on;
  `ADD_ON_COMPLETE` once per add-on (tool `sweep_alerts`, #37; tested).
...
- `LARVA_HATCHED` once per inject, a few steps before the larva are seen, and never for larva a hatchery makes itself
  (tool `sweep_alerts`, #37).
- `MINERALS_EXHAUSTED` once per field this player mined, in the observation it disappears from (8 of 8, at the same
  steps); `VESPENE_EXHAUSTED` once per geyser (2 of 2); `MULE_EXPIRED` in the observation that reports the MULE
  dead; `NUKE_COMPLETE` for a nuke armed at a ghost academy (tool `sweep_alerts`, #37; tested).
- `NUCLEAR_LAUNCH_DETECTED` once per enemy nuke, whether aimed at this player's base or somewhere it sees nothing, in
  the first observation after the launch; `NYDUS_WORM_DETECTED` once per enemy worm, seen or not. This player's own
  nuke raises nothing, and its own worm only `BUILDING_COMPLETE` (tool `sweep_alerts`, #37).
- `UNIT_UNDER_ATTACK` and `BUILDING_UNDER_ATTACK` come only for units the camera does not show (5 of 5 off screen,
  0 of 5 on), once per unit, and not again for that unit until it has gone some 6000 to 6500 steps without being
...
- The next observation reports every order the game carried out, stepped or realtime, with the step it was carried
  ...
  `Success` but not carried out, and one ignored as identical to a unit's first are not (tool `sweep_orders`).
...
- One terrain-height byte is an eighth of a unit, and 127 is height 0: bytes 191, 207, 223 and 239 are heights 8,
  ...
  0.41 off elsewhere in the tile, and beside a cliff it can be the other side's level (#17).
- Ramp corner heights fall on quarters (byte 193 is 8.25, 205 is 9.75). Flat ground stands near its level, though
  Pylon's main base is at 12.15 over byte 223 (#17; corpus).
- Next to a cliff each corner holds one level: a tile whose corners split 3 to 1 stands with the three (455 of 457),
  and one split 2 to 2 on the upper side (115 of 115). Corners across a ramp differ by at most 1.25, across a cliff
  by at least 2, and ground levels are 2 apart (#17).
- Torches, Ultralove and Incorporeal have ramps standing up to 0.76 above the heights the game sends. On four ramps
  of the pool, tiles in one row across differ by half a byte, and on every measured ramp the next row up is 0.1875
  higher (#17, #22).
- A unit's z is within 0.05 of its tile center's height 9 times in 10, and within 0.03 of the interpolated height
  (#17).
...
- A structure's center snaps to the grid, to a tile corner for an even footprint and a tile center for an odd one.
  The game moved a forge placed at the edge of buildable ground from (93, 150) to (90.5, 147.5), and a pylon ordered
...
- The game rounds supply used and army supply down: one zergling left `food_used` at 14, a second made it 15, and a
  third left it at 15 (#28; tested). `food_used` counts units in production, `food_workers` does not; supply left
  goes negative once supply providers are lost (stated).
...
- The score's idle production and idle worker times are in Normal seconds, summed over the idle units (two idle
  drones added 20 over 160 steps), and all 10,232 values in the corpus are whole steps (#28; corpus).
- The score is the value of every unit and structure, finished or not, plus the resources banked; `total_value`
  counts only what is finished, `collection_rate` is per minute, and `spent` counts an order when it is queued and
  refunds it when cancelled. `used_*` leaves out what was destroyed, and `total_used_*` counts it (stated).
- Recent APM reads 0 through the raw interface (#28).

## The computer opponent

- The very easy computer keeps the game going, leaves the player alone and does not come to the map's middle early,
  but it does attack at some point (tool `sweep_buffs`, tool `sweep_tech_tree`).
- It concedes a game it sees as lost, with nothing left to fight with or its base cleared, and the game ends (tool
  `sweep_alerts`).
- A bare api against the very hard zerg lasts over 60 s and ends in victory or defeat (tested). Against a bot that
  never leaves its base, the computer's army shows up but none of its buildings, and it researches, as its units'
  levels show (corpus).

## Not understood yet

- **A structure that dies just as vision of it lapses**, before the game swaps it for a copy in the fog, is neither
  reported dead nor listed as a copy, so NachOS keeps it, stale, and never learns it died (#37).
