...
- **A bot subscribes handlers to an `Api` rather than subclassing `BotAI`**, and nothing is `async`. `on_step` and
  the other hooks become events, and a handler selects the ones it wants ([Events](#events),
  [events.md](events.md)).
- **Game time is counted in steps**, the game loop, never in iterations. A turn is one step unless
...
- **Your own units are `OwnUnit`s**, the only ones with orders, cargo and weapon cooldown. A unit's type can be a
  type parameter: `units.own.of_type(UnitType.Marine)` is typed as your marines ([Units](#units)).
- **Ids use the names players use**, `CONCUSSIVE_SHELLS` for `PUNISHERGRENADES`, cover only what a melee game
...
- **A unit's weapons, speed and armor include its owner's upgrades**, the enemy's too, and speeds are per second of
...
- **Nothing is subclassed, and nothing is `async`.** Construct an `Api`, at import time if you like, and hand it
  to a runner with the race it plays: `run_local("PylonAIE_v4", ApiBot(api, Race.TERRAN), Computer(Race.ZERG))`.
- **A turn is one step unless you say otherwise.** `run_local(..., steps_per_turn=4)` matches python-sc2, whose
  `client.game_step` defaults to 4. It is set for a game and cannot be changed during one.
- **`Computer()` defaults to very hard, with a random race and build.** python-sc2's `Computer` requires a race
  and defaults to easy.
- **A game holds one bot and at most one computer.** Every current map has two slots, and the game silently drops
  extra players. Two bots cannot share a process.
- **`run_local`'s `time_limit` calls the game a tie at the first turn at or past the limit.** python-sc2's
  `game_time_limit` waits for the first turn strictly past it. `run_ladder` takes no limit: a bot ends a ladder
  game early only by leaving it, which concedes it.
...
- **New: recordings.** Both runners accept `record_to=path`, which writes the whole conversation with the game to
  `path`. A `Client` over `PlaybackTransport(Recording(path))` then plays it back with no game running. A run
  killed before it finishes leaves only part of the file.

## Events

[events.md](events.md) lists every event, what selects it, and when it comes.

- **A bot's code runs in handlers, not overridden methods.** `on_start`, `on_step` and `on_end` are handlers of
  `GameStartEvent`, `TurnEvent` and `GameEndEvent`, subscribed with `@api.events.on(TurnEvent)`. A module-level
  function is subscribed as it is defined. A method is marked, and subscribed when an instance is passed to
  `api.events.subscribe(instance)`, usually by its own `__init__`.
- **Keep `on_step` as one `TurnEvent` handler** that calls the bot's parts in the order it wants. Handlers run by
  priority, highest first, then in the order they subscribed, which is hard to follow across many modules.
...
- **The last observation gets no turn, as in python-sc2, but it is taken in.** A `GameEndEvent` handler reads the
  game as it ended; python-sc2's `on_end` sees the observation before it.
- **A handler cannot be `async`.** One that is is refused when it subscribes.
- **Subscriptions outlive a game.** A function stays subscribed for the life of the api, and an instance until it
  is passed to `api.events.unsubscribe`; both are held strongly. A bot that makes new objects for each game
...
- **The `on_unit_*` hooks are events too**, handed out each turn between `TurnStartEvent` and `TurnEvent`, in the
  order [events.md](events.md) gives, and made only for the types something subscribes to:
...
- **A handler selects the events it wants instead of testing each one.** `AlertEvent.only(Alert.X)`,
  `UnitDiedEvent.only(UnitType.Structure)` and `on(OwnUnitDamagedEvent).where(predicate)` replace the `if` at the
  top of a python-sc2 hook. `once` and `every_steps` then count only the selected events, and with `only` NachOS
  does not make the rest. What python-sc2 leaves the bot to compare every step, such as a caster's energy, is an
...
- **Priority orders a whole turn, not one event.** A turn's events go out priority first: every `HIGH` handler of
  ...
  before `on_step`; a bot keeps that by leaving its handlers at one priority.
- **An event takes its step by keyword.** One a bot emits without it gets the game's.
- **A handler of a class receives the events of its subclasses.** A handler of `Event` receives every event, and
  makes NachOS produce every type of event. `UnitEvent`, `BuffEvent`, `VitalEvent` and `AreaEvent` each cover a
  whole kind, this player's units and the enemy's alike.
- **A bot can define its own events** by subclassing `Event`, and send them with `api.events.emit`. python-sc2
...
- **Damage is what a unit lost since the previous observation**, less what it regained in between, and a unit that
...
- **Everything NachOS raises for a failure of its own is a `NachOSError`.** python-sc2's `ProtocolError` with
  `is_game_over_error` true is `GameEndedError` here, and its `ConnectionAlreadyClosedError` is
...
- **`api.time` differs from python-sc2's `time` in the last bit on about a quarter of steps.** Neither 22.4 nor
  1 / 22.4 is exact in binary. python-sc2 divides by 22.4 and NachOS multiplies by 1 / 22.4, and the two round
  differently. NachOS lands exactly on every whole second. python-sc2 overshoots the whole seconds in the top
  quarter of each power of two, so 15 seconds comes out as `15.000000000000002` and 60 as `60.00000000000001`. A
  test that compares the two needs a tolerance.
...
- **Two buffs are not the ids python-sc2's names suggest.** Concussive shells put `SLOW` on their target, never
  ...
  named after the unit that causes it, so `STIMPACK` is `MARINE_STIMMED`.
- **The curated enums hold only what a melee game needs.** Calling an enum on an id it leaves out raises
  `ValueError`, as with any enum; NachOS reads ids with `read`, which raises `UncuratedIdError`, naming the id and
  what the raw catalog calls it. `sc2nachos.ids.raw` holds every id, under Blizzard's own names.
- **An uncurated id stops the game as the observation comes in** when it is a unit's type or an upgrade this
  player holds: both belong among the curated ids, so a game that reports one is a gap to fill. A buff, an effect or
...
- **Grids are indexed `[x, y]`.** python-sc2's `PixelMap.data_numpy` is indexed `[y, x]`. `grid[point]` reads the
  tile any point falls in, where a `PixelMap` needs whole-number coordinates. Reading past the edge raises
...
- **A unit is one object under one id for the whole game.** The game's tags do not identify a unit: each time a
  structure goes out of sight it is replaced by a remembered copy under a new tag, and a mineral field starts the
  game as one. NachOS follows a structure through those swaps, so it keeps its object and id, and `unit.tag` is
  only the game's current handle. An id's first digit is the unit's alliance when first seen, 1 for yours and 4 for
  the enemy's; the rest counts that alliance's units. python-sc2 builds new objects every step, keyed by tag.
...
- **A unit refers to other units by object.** An order's target, a rally target, a passenger, an add-on and an
  engaged target are units, stale or dead ones included; python-sc2 gives tags to look up. An order aimed at
  nothing has `None` for its target; python-sc2's is `0`.
- **`builder` and `construction` link a structure and what builds it.** `structure.builder` is the SCV building it
  ...
  it is halted, and for a Protoss structure. python-sc2 has `is_constructing_scv` and no link; it is true from the
  order on, while the SCV walks to the site, whereas `construction` is `None` until the structure is placed, so an
  SCV on its way is one whose first order is the build ability.
...
- **What the game never showed raises `NotReportedError`**: the health of a cloaked enemy never detected, or the
  contents of a mineral field no one has looked at. python-sc2 answers 0.
...
- **A unit's type is a type parameter too.** `UnitType` has a class for every type, `UnitType.Marine`, whose `id`
  ...
  `UnitType.ProtossStructure`. `Unit[UnitType.Marine]` is a marine, and a read only some types have is an error on
...
- **A unit's weapons, speed and armor include its owner's upgrades.** Yours come from the observation, the enemy's
  ...
  `calculate_speed(upgrades)` for anyone else's, and its `ground_range`, `ground_dps` and `armor` include none. For
  another set of upgrades, read `unit.type_data.with_upgrades(set)`.
- **The enemy's upgrades are read off its units, once, for the whole player.** The game reports nothing of the
  enemy's upgrades beyond the attack, armor and shield levels on each unit in sight, and those belong to the player
  and the unit's line: a marine at attack level 2 puts the first two Terran Infantry Weapons into
  `api.enemy.upgrades`, and every enemy marauder then counts them, in the fog as much as in sight. An ultralisk's
  armor is left out, since 2 could be two levels or Chitinous Plating. Anything the game never reports, such as
  Metabolic Boost, a bot adds with `api.enemy.assume_upgrades` and takes back with `forget_upgrades`; with
  `Api(enemy_upgrade_inference=UpgradeInference.NONE)` NachOS adds nothing itself. With
  ...
  shells or matrixed, and an enemy storm. python-sc2 leaves all of this to the bot, per call.
- **`unit.armor` is what a unit in sight reports, so it is exact.** For a unit out of sight it is what the enemy is
  known to have.
- **`unit.shield_armor` is the shields level its owner has**, which python-sc2 writes as
...
- **`unit.speed` counts no creep and no buff yet**; python-sc2's `real_speed` counts both.
- **Speeds are per second of the game's Faster speed, as `velocity` is.** The game's tables give them per second of
  its Normal speed, 16 steps, which python-sc2 hands on, so its code multiplies by 1.4 to get the distance a unit
  covers in a real second. NachOS has done that already: a zergling's `speed` is 4.13, not 2.95.
...
- **A collection never changes.** python-sc2's `Units` is a list you can append to; NachOS's is a fixed sequence,
  and every filter returns a new one.
- **Collections are combined with `Units.combined(a, b, ...)`**, which keeps each unit where it first appears and
  knows a unit by its NachOS id. python-sc2 spells it `a | b` and `a + b`.
- **Your own data about a unit is keyed by the unit or its id.** A unit takes no attributes of yours, and its class
  is not yours to subclass. A `weakref.WeakKeyDictionary` keyed by unit lets go of an entry once the unit is dead
  and dropped.
...
| `bot.minerals`, `vespene` | `api.resources.minerals`, `vespene`, and `api.resources.covers(cost.resources)` against a cost |
...
- **`api.effects` holds only what the game reports as an effect.** python-sc2 adds force fields and reaper
  grenades to `state.effects`; in NachOS they are units.
...
- **The grids refuse writes.** python-sc2 rebuilds the pathing grid every step, so a write into it lasted one
...
- **A ramp's ends are the tiles within a byte of its highest and lowest, and come out the same size whichever
  way it faces.** python-sc2's `upper` and `lower` are the tiles sharing the highest and lowest terrain byte, read
  at each tile's lower left corner, so a ramp and its mirror image give ends of different sizes -- 13 and 6 tiles
  for two halves of the same map. NachOS reads the height at the tile's center, which is symmetric, and allows a
  byte because a row straight across a ramp is not quite level where the corners under it differ.
- **A patch of ground is a ramp as a whole, and no patch is dropped for being small.** python-sc2 asks of each
  tile alone whether the nine terrain bytes around it are equal, calls it a ramp point if they are not, and then
  throws away any group of fewer than 8. NachOS groups the ground a unit can walk over but cannot build on and
  reads the whole patch: one whose heights span half a level or more climbs from one level to the next, and is a
  ramp. On the 2026 ladder pool the two find the same ramps, tile for tile.
- **There is no `vision_blockers`, because the map's grids cannot tell.** Nothing in `ResponseGameInfo` tells a
...
- **A table is keyed by the id itself.** python-sc2 keys by the number inside it, so every lookup reads
  `units[UnitTypeId.MARINE.value]`.
- **No row carries a name**: the curated id is a better one than the game gives. A unit type, upgrade or effect
...
- **A field naming something uncurated reads `None`, and `tech_aliases` drops it, except where an override names
  what works.** The rows of a lurker, a baneling, a rich refinery, an auto-turret, a locust and a purification
  ...
  builds and so on; `gamedata/_techtree/_overrides.py` lists them with reasons, and the tech tree sweep checks them
  in game each time it runs (`docs/curating-ids.md`). python-sc2 papers over only the lurker, by writing
  `MORPH_LURKER` into the message it was handed. The rest name an ability nothing can order, such as a changeling's
...
- **A cost is minerals, vespene and supply; times are steps beside it.** python-sc2's `Cost` carries a `time` in
  ...
  are their own fields. What a player holds is `Resources`, minerals and vespene alone, which adds, subtracts,
  scales, divides and answers `covers`; `cost.resources` is what to hold against it. Its amounts are fractional,
  since half a cost and an average cost are ordinary things to want; what the game gave stays whole until something
  divides it.
- **A unit type's cost is everything spent to reach it; an ability's is what the game charges.** An orbital
  ...
  morph alone, so `unit_data.cost` hands the game's number back as it is and `morphed_from` says what to subtract.
  `ability_data.cost` has it subtracted already: `COMMAND_CENTER_MORPH_ORBITAL_COMMAND` charges 150, an extractor
...
- **The relations between the tables were swept in game, not read from the game's files**, since `RequestData`
  ...
  asks the game what each unit type is offered and sees what goes when a structure dies or an upgrade finishes
  (`docs/curating-ids.md`). Where the two disagree, the game says a probe is offered a gateway and a forge once a
  nexus stands, whatever the pylons, and a roach a ravager once a roach warren stands, where python-sc2 says a
  pylon and a hatchery.
- **Requirements belong to a unit type and an ability together**, since one ability can need different things of
  ...
  to move. `units[t].ability_requirements` holds an entry for every ability `units[t]` is offered, even one that
  needs nothing. A required structure counts as standing where a type whose `tech_aliases` name it stands, and an
...
- **What an upgrade changes was swept in game too, and is added, not multiplied.** python-sc2 writes
  ...
  `tools/sweep_upgrades.py` read off the game's rows after each upgrade, plus every upgrade that raises a level the
  type's units report without changing the row: a shields level for every protoss type, and the attack levels for
  a void ray, a carrier and a sentry, which the rows give no weapon. Those rows take in weapon damage, bonuses and
  range, armor and speed, and nothing else, not attack speed, and Anabolic Synthesis counts there whether or not
  the ultralisk is on creep. Where the two disagree, the game says a cyclone's attack level adds 1 where python-sc2
  says 2; Flux Vanes, Gravitic Boosters, Muscular Augments and Anabolic Synthesis speed up by other factors;
...
- **A row is as it stands before any upgrade, unless `with_upgrades` made it.** Asked again later in a game, the
  game folds in the asking player's upgrades, and only that player's, though a unit type has one row; NachOS asks
  once.
- **Reading the tables leaves the message they came from alone.** Building python-sc2's `GameData` writes
  `MORPH_LURKER` over the lurker row in the `ResponseData` it was handed, so whatever reads that message afterwards
  sees the substitution rather than what the game said.
