...
class Alert(ReadableIntEnum):
    ...
    own worm raises only `BUILDING_COMPLETE` (in game)."""
    ...
    """A mothership of this player's has finished, without `TRAIN_UNIT_COMPLETE` (in game)."""
    ...
    charge, one alert for each (in game)."""
    ...
    zerglings. A mothership, a warp-in and a morph raise alerts of their own (in game)."""
